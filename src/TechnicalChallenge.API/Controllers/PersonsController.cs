...
[Authorize]
public class PersonsController(IMediator mediator) : BaseApiController
{
    [HttpGet]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<IActionResult> GetAll(CancellationToken cancellationToken)
    {
        var result = await mediator.Send(new GetAllPersonsQuery(UserId), cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await mediator.Send(new GetPersonByIdQuery(id, UserId), cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await mediator.Send(new DeletePersonCommand(id, UserId), cancellationToken);
